...
import bigLogo from "../../logos/big_logo1.png";
import smallLogo from "../../logos/second.png";
import "./Header.css";
function Header() {
  const { userData, setUserData } = useContext(Context);
  const [windowSize, setWindowSize] = useState(window.innerWidth > 500);
  ...
  return (
    <Navbar collapseOnSelect bg="light" variant="light" className="m-0 p-0">
      <div className="container">
        <Navbar.Brand>
          <NavLink className="navbar-brand mt-2" to="/">
            {windowSize ? (
              <img src={bigLogo} alt="college-bazaar" className="bigLogo" />
            ) : (
              <img src={smallLogo} alt="college-bazaar" className="smallLogo" />
            )}
          </NavLink>
        </Navbar.Brand>
